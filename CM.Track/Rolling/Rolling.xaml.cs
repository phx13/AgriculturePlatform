...
{
    /// <summary>
    ///     RollingControl.xaml 的交互逻辑
    /// </summary>
    public partial class Rolling : ChartViewBase
    {
        #region 构造函数
        ...
        public Rolling(ChartViewBaseModel model)
            : base(model)
        {
            InitializeComponent();
            m_Controller = (RollingController)Controllers[0];
            DataContext = m_Controller;
            Loaded += RollingControl_Loaded;
        }
        ...
        /// <param name="e"></param>
        private void RollingControl_Loaded(object sender, RoutedEventArgs e)
        {
            if (m_IsLoaded)
            {
            }
        ...
        ///     控制器
        /// </summary>
        private readonly RollingController m_Controller;
